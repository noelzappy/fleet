...
func TestLoadExample(t *testing.T) {
	...
	tests := []struct {
		...
		{"profile count", len(f.Profiles), 8},
		{"codex floor", f.Harnesses["codex"].MinVersion, "0.39.0"},
...
func TestApplyDefaults(t *testing.T) {
	...
	tests := []struct {
		...
		{"isolation", Fleet{}, func(f *Fleet) (any, any) { return f.GitHub.Isolation, "box" }},
		{"idle grace", Fleet{}, func(f *Fleet) (any, any) { return f.Routing.IdleGrace, "5m" }},
		{"quota cooldown", Fleet{}, func(f *Fleet) (any, any) { return f.Routing.QuotaCooldown, "5h" }},
...
func TestValidate(t *testing.T) {
	...
	tests := []struct {
		...
		{"unknown kind", func(f *Fleet) { f.Harnesses["cc"] = Harness{Kind: "aider"} }, "not one of"},
		...
		{"isolation project ok", func(f *Fleet) { f.GitHub.Isolation = "project" }, ""},
		{"bad isolation", func(f *Fleet) { f.GitHub.Isolation = "vm" }, "github.isolation"},
		{"bad idle grace", func(f *Fleet) { f.Routing.IdleGrace = "soon" }, "idle_grace"},
		{"bad quota cooldown", func(f *Fleet) { f.Routing.QuotaCooldown = "later" }, "quota_cooldown"},
		{"zero quota cooldown", func(f *Fleet) { f.Routing.QuotaCooldown = "0s" }, "quota_cooldown"},
